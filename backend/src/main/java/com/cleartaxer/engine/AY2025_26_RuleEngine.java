...
public class AY2025_26_RuleEngine extends AbstractTaxRuleEngine {

    public static final String VERSION = "AY2025-26-v1.0.0-Budget2024";
    public static final double STANDARD_DEDUCTION_OLD = 50000.0;
    public static final double STANDARD_DEDUCTION_NEW = 75000.0; // Increased to 75k in Budget 2024

    private final DeductionCalculator deductionCalculator;

    @Override
    public AssessmentYear getAssessmentYear() {
        return AssessmentYear.AY_2025_26;
    }
    ...
    public TaxCalculationResult calculateOldRegime(TaxCalculationRequest request) {
        ...
        if (deductions.getHomeLoanInterestSelfOccupied() > 0) {
            ...
            trace.add(AuditTraceStep.builder()
                    ...
                    .description("Interest deduction on housing loan (max ₹2,00,000)")
        ...
        trace.add(AuditTraceStep.builder()
                ...
                .description("Sum of all income heads")
                .amount(grossTotalIncome)
                .formattedDetails(String.format("Gross Income: ₹%,.0f", grossTotalIncome))
                .build());

        // 2. Standard Deduction
        double standardDeductionAllowed = (grossSalary > 0) ? Math.min(grossSalary, STANDARD_DEDUCTION_OLD) : 0.0;
        if (standardDeductionAllowed > 0) {
            trace.add(AuditTraceStep.builder()
                    .stepNumber(stepNum++)
                    .stepName("Standard Deduction")
                    .ruleReference("Section 16(ia)")
                    .description("Flat standard deduction for salaried individuals (Old Regime)")
        ...
        trace.add(AuditTraceStep.builder()
                ...
                .description("Taxable income rounded to nearest ₹10")
        ...
        trace.add(AuditTraceStep.builder()
                ...
                .description(String.format("Old Regime progressive tax slabs for %s", ageCategory.getDescription()))
                .amount(taxOnSlabs)
        ...
        if (netTaxableIncome <= 500000.0 && taxOnSlabs > 0) {
            ...
            trace.add(AuditTraceStep.builder()
                    ...
                    .description("Full tax rebate for income <= ₹5,00,000 (Max ₹12,500)")
                    .amount(rebate87A)
        ...
        if (surchargeResult.amount() > 0) {
            trace.add(AuditTraceStep.builder()
                    ...
                    .description(String.format("Surcharge with marginal relief: ₹%,.0f", surchargeResult.marginalRelief()))
    ...
    public TaxCalculationResult calculateNewRegime(TaxCalculationRequest request) {
        ...
        // 1. Gross Total Income
        double grossSalary = income.getGrossSalary();
        double housePropIncome = Math.max(0, income.getIncomeFromHouseProperty());
        ...
        trace.add(AuditTraceStep.builder()
                .stepNumber(stepNum++)
                .stepName("Gross Total Income (New Regime AY 2025-26)")
                .ruleReference("Section 115BAC")
                .description("Sum of all eligible income heads")
                .amount(grossTotalIncome)
                .formattedDetails(String.format("Gross Income: ₹%,.0f", grossTotalIncome))
                .build());

        // 2. Standard Deduction: ₹75,000 for AY 2025-26
        double standardDeductionAllowed = (grossSalary > 0) ? Math.min(grossSalary, STANDARD_DEDUCTION_NEW) : 0.0;
        if (standardDeductionAllowed > 0) {
            trace.add(AuditTraceStep.builder()
                    .stepNumber(stepNum++)
                    .stepName("Standard Deduction (Enhanced)")
                    .ruleReference("Section 16(ia) - Budget 2024")
                    .description("Enhanced Standard Deduction of ₹75,000 for salaried employees in AY 2025-26")
                    .amount(standardDeductionAllowed)
                    .formattedDetails(String.format("Standard Deduction: ₹%,.0f", standardDeductionAllowed))
                    .build());
        }

        // 3. Permissible Deductions (Employer NPS 80CCD(2))
        ...
        trace.add(AuditTraceStep.builder()
                ...
                .stepName("Net Taxable Income")
                .ruleReference("Section 288A")
                .description("Gross Income minus Enhanced Standard Deduction minus 80CCD(2)")
                .amount(netTaxableIncome)
                .formattedDetails(String.format("Taxable Income: ₹%,.0f", netTaxableIncome))
                .build());

        // 5. Revised New Regime Slabs (Budget 2024 for AY 2025-26)
        List<SlabBreakdown> slabBreakdowns = calculateNewRegimeSlabs(netTaxableIncome);
        double taxOnSlabs = slabBreakdowns.stream().mapToDouble(SlabBreakdown::getTaxForSlab).sum();

        trace.add(AuditTraceStep.builder()
                .stepNumber(stepNum++)
                .stepName("Tax on Revised Slabs (Budget 2024)")
                .ruleReference("Section 115BAC(1A) Revised")
                .description("AY 2025-26 Slabs: 0-3L (0%), 3-7L (5%), 7-10L (10%), 10-12L (15%), 12-15L (20%), >15L (30%)")
                .amount(taxOnSlabs)
                .formattedDetails(String.format("Tax before Rebate: ₹%,.0f", taxOnSlabs))
                .build());
        ...
        if (netTaxableIncome <= 700000.0 && taxOnSlabs > 0) {
            ...
            double excessOver7L = netTaxableIncome - 700000.0;
            if (taxOnSlabs > excessOver7L) {
                rebate87A = taxOnSlabs - excessOver7L;
                trace.add(AuditTraceStep.builder()
                        .stepNumber(stepNum++)
                        .stepName("Section 87A Marginal Relief")
                        .ruleReference("Section 87A Proviso")
                        .description(String.format("Tax cannot exceed excess income over ₹7L (₹%,.0f). Marginal relief: ₹%,.0f",
                                excessOver7L, rebate87A))
                        .amount(rebate87A)
    ...
    public List<SlabBreakdown> calculateNewRegimeSlabs(double taxableIncome) {
        ...
        // S2: 3L - 7L (5%) - Budget 2024 widened slab
        double s2 = Math.max(0, Math.min(taxableIncome, 700000.0) - 300000.0);
        slabs.add(SlabBreakdown.builder()
                .slabRange("₹3,00,001 - ₹7,00,000")
                .minIncome(300000.0).maxIncome(700000.0)
                .taxableAmountInSlab(s2).ratePercentage(5.0).taxForSlab(s2 * 0.05).build());

        // S3: 7L - 10L (10%)
        double s3 = Math.max(0, Math.min(taxableIncome, 1000000.0) - 700000.0);
        slabs.add(SlabBreakdown.builder()
                .slabRange("₹7,00,001 - ₹10,00,000")
                .minIncome(700000.0).maxIncome(1000000.0)
                .taxableAmountInSlab(s3).ratePercentage(10.0).taxForSlab(s3 * 0.10).build());

        // S4: 10L - 12L (15%)
        double s4 = Math.max(0, Math.min(taxableIncome, 1200000.0) - 1000000.0);
        slabs.add(SlabBreakdown.builder()
                .slabRange("₹10,00,001 - ₹12,00,000")
                .minIncome(1000000.0).maxIncome(1200000.0)
